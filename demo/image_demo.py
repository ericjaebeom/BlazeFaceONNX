"""Single-image BlazeFace end-to-end ONNX inference demo."""
...
def parse_args():
    p = argparse.ArgumentParser(description="BlazeFace E2E single-image demo")
...
def draw_detections(image_bgr, detections, scale, pad_top, pad_left, model_h, model_w):
    ...
    for det in detections:
        ...
        cv2.rectangle(image_bgr, (ox1, oy1), (ox2, oy2), (0, 255, 0), box_thick)
        cv2.putText(
            image_bgr, f"{score:.2f}", (ox1, oy1 - 6),
        ...
        for i in range(6):
            ...
            kx = int(kp_x / scale)
            ky = int(kp_y / scale)
            cv2.circle(image_bgr, (kx, ky), kp_radius, (0, 0, 255), -1)
...
def main():
    ...
    tensor, scale, pad_top, pad_left = preprocess(image_bgr, model_h, model_w)
    ...
    draw_detections(image_bgr, detections, scale, pad_top, pad_left, model_h, model_w)

    cv2.imshow("BlazeFace Detection", image_bgr)
